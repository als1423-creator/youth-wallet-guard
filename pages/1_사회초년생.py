...
if MSG_KEY not in st.session_state:
    st.session_state[MSG_KEY] = [{"role": "assistant", "content": f"안녕하세요! {CATEGORY}을 위한 정책 정보를 안내해 드립니다. 어떤 혜택이 궁금하신가요?"}]
...
if prompt := st.chat_input(f"{CATEGORY} 지원 정책에 대해 물어보세요 (예: 청년도약계좌 알려줘)"):
        ...
        with st.spinner("정부 공공 데이터망에서 최신 정책을 가져오는 중입니다..."):
            try:
                ...
                params = {
                    ...
                    "display": "5",       # 5개만 가져오기
                    "pageIndex": "1",
                    "query": prompt       # 사용자가 입력한 질문(키워드)으로 검색
                }
                
                response = requests.get(url, params=params)
                
                # 2. 📝 구닥다리 XML 데이터를 딕셔너리로 파싱
                ...
                try:
                    ...
                    if not isinstance(policies, list):
                        policies = [policies] # 결과가 1개일 경우 리스트로 묶어줌
